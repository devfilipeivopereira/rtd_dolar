...
- `DOM`, `Radar` e `Score Quant` devem usar cache de derivados por ativo/snapshot/CSV/fluxo; `Sistema` ou `Revisar` deve mostrar `Cache derivados` com hits/misses.
- DOM, Book e Times devem evitar `innerHTML` repetido quando o HTML gerado nao mudou; `Sistema` ou `Revisar` deve mostrar `Paint paineis` com aceitos/pulados.
...
Esse check falha se a hotbar perder o estado da tela ativa, os badges `P/B/T/CSV/Flow/Edge`, o botao `Proximo`, o throttle de atualizacao ou o reuso da logica de `Proximo passo` do `Painel`.
