...
- A hotbar deve espelhar a aba ativa, mostrar a trilha `Grupo / Tela`, estado operacional da tela ativa, botao `Proximo` e permitir troca rapida para Monitor, DOM, Book, T&T, Fluxo, Oportunidades, Ativos, Conexoes e Sistema.
...
Para validar que a hotbar continua guiando o fluxo de trabalho:

```text
node tools/validate-hotbar-workflow.js
```
...
Esse check falha se a hotbar perder o estado da tela ativa, o botao `Proximo`, o throttle de atualizacao ou o reuso da logica de `Proximo passo` do `Painel`.
...
5. O botao `Proximo` da hotbar abre a mesma tela indicada pelo roteiro do `Painel`.
6. `Alt+1` a `Alt+9` continuam abrindo as telas frequentes quando o foco nao esta em campo de texto.
