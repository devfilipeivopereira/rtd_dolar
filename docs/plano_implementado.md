...
- Skip de paint por painel para DOM, Book e Times, evitando `innerHTML` repetido quando a tabela/escada visual nao mudou.
...
- Validador `tools/validate-live-render-scheduler.js` para preservar coalescing de render, motivos, ativos, filtro por aba ativa, `Render Guard` e pausa em aba oculta.
...
19. Confirmar `Latencia WS`, `Msg/s`, `Render UI`, `Render guard`, `Render background` e contadores no `Sistema`.
...
29. Rodar `node tools/validate-hotbar-workflow.js`.
30. Confirmar `GET /bootstrap` retornando health, assets, snapshot, flow e signals.
...
38. Confirmar `Feed`, `Idade backend`, `Feed selecionado` e Feed por ativo em tempo real.
39. Confirmar que `Score Quant` e `Radar` reduzem confianca quando o feed fica `Atrasado` ou `Parado`.
